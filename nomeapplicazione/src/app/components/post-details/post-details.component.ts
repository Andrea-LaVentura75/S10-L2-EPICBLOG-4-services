...
export class PostDetailsComponent {
  ...
  ngOnInit() {
    this.route.params.subscribe((params) => {
      ...
      // Trova il post corrispondente all'id nei parametri
      const found = this.posts.find((p) => p.id === parseInt(params['id']));

      if (found) {
        this.foundPost = found; // Aggiorna la proprietà di classe con il post trovato
        console.log('Post trovato:', this.foundPost);
      } else {
